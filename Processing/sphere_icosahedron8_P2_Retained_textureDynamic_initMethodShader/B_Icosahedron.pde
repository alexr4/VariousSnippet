...
class Icosahedron {
  ...
  void createWireframeShape()
  {
    ...
    for (int i=0; i<positions.size(); i += 3) {
      ...
      float h = 0;//map(a_.x, limiteB, limiteH, 130, 230);
  ...
  void createUvMap()
  {
    ...
    for (int i=0; i<texCoords.size(); i+=3)
    {
      ...
      loc0.x = map(loc0.x, 0, 1, 10, uvMapImg.width);
      loc0.y = map(loc0.y, 0, 1, 10, uvMapImg.height);
      loc1.x = map(loc1.x, 0, 1, 10, uvMapImg.width);
      loc1.y = map(loc1.y, 0, 1, 10, uvMapImg.height);
      loc2.x = map(loc2.x, 0, 1, 10, uvMapImg.width);
      loc2.y = map(loc2.y, 0, 1, 10, uvMapImg.height);
