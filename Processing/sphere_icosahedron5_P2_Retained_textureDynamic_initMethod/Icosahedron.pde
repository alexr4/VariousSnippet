...
class Icosahedron {
  ...
  Icosahedron(int level_, float r_, PImage tex_) {
    ...
    icosahedron = createShape(PShape.GROUP);
    normalShape = createShape(PShape.GROUP);
    wireframeShape = createShape(PShape.GROUP);

    createIcosahedron();
    println("\ticosahedron texture shape has been created");
    ...
    for(int i = 0; i<positions.size(); i++)
     {
     println("\tvector position("+i+")"+positions.get(i));
     println("\tvector texCoords("+i+")"+texCoords.get(i));
     }*/
  }
  ...
  void addVertex(PVector p, PVector n, PVector t) {
    ...
    t.x = map(t.x, 0, 1, 0, textureWidth);
    t.y = map(t.y, 0, 1, textureHeight, 0);
    texCoords.add(t);
  }
  ...
  { 
    for (int i=0; i<positions.size(); i += 3) {
      ...
      face.noStroke();
      face.texture(tex);
      face.vertex(a_.x, a_.y, a_.z, ta_.x, ta_.y);
      face.vertex(b_.x, b_.y, b_.z, tb_.x, tb_.y);
      face.vertex(c_.x, c_.y, c_.z, tc_.x, tc_.y);
      face.endShape();

      icosahedron.addChild(face);
    }
  }
