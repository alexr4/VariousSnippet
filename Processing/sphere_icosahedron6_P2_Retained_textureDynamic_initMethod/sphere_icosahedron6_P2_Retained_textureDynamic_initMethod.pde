...
/*
  for (int i=0; i<textCoord.size(); i+=3)
  {
    ...
    loc0.x = map(loc0.x, 0, 1, 0, uvMap.width*0.85);
    loc0.y = map(loc0.y, 0, 1, 0, uvMap.height*0.85);
    loc1.x = map(loc1.x, 0, 1, 0, uvMap.width*0.85);
    loc1.y = map(loc1.y, 0, 1, 0, uvMap.height*0.85);
    loc2.x = map(loc2.x, 0, 1, 0, uvMap.width*0.85);
    loc2.y = map(loc2.y, 0, 1, 0, uvMap.height*0.85);


    uvMap.fill(127);
    uvMap.noStroke();
    uvMap.text("ta.x = "+ta.x, loc0.x+2, loc0.y, 1);
    uvMap.text("tb.x = "+tb.x, loc1.x+2, loc1.y+12, 1);
    uvMap.text("tc.x = "+tc.x, loc2.x+2, loc2.y+22, 1);
  }
*/
  uvMap.endDraw();
