...
class Icosahedron {
  ...
  { 
    for (int i=0; i<positions.size(); i += 3) {
      ...
      ta_.sub(origine); //substract a to o
      ta_.normalize(); //normalize
      ta_.mult(r); //multiply by radius

      tb_.sub(origine); //substract a to o
      tb_.normalize(); //normalize
      tb_.mult(r); //multiply by radius

      tc_.sub(origine); //substract a to o
      tc_.normalize(); //normalize
      tc_.mult(r); //multiply by radius
      ...
      beginShape();
      texture(tex2);
      vertex(a_.x, a_.y, a_.z, ta_.x, ta_.y);
      vertex(b_.x, b_.y, b_.z, tb_.x, tb_.y);
      vertex(c_.x, c_.y, c_.z, tc_.x, tc_.y);
      endShape();
    }
  }
