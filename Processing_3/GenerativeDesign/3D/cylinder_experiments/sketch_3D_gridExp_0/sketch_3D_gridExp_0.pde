...
void setup()
{
  ...
  for (int i = 0; i<gridDepth/resD; i++)
  {
    ...
    float noiseY = map(noise(xInc), 0, 1, -100, 100);
    float noiseX = map(noise(i), 0, 1, -100, 100);
    
    for (int j = 0; j<maxAngle/resAngle; j++)
    {
      float phi = map(j, 0, (maxAngle/resAngle)-1, 0, radians(maxAngle));
      ...
      {
        noise = map(noise(xInc, yInc, zInc), 0, 1, -100, 100);
      }
      
      radius[i][j] = 100;//+noise;

      float x = cos(phi)*radius[i][j];
      float y = sin(phi)*radius[i][j];
