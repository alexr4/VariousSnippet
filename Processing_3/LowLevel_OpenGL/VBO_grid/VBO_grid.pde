...
void setup() {
  ...
  pointSize = 1;
  anchorPoint = new PVector(width/2, height/2, -500);
  initGrid(1, width*2, height*2);
  computeGridPosition();
  createGridGeometry();
  initVBO();
...
void draw() {
  ...
  surface.setTitle("FPS : "+floor(frameRate)+" Nb particles : "+gridList.size()+" per cube");
  background(0);
  ...
  offscreen.translate(anchorPoint.x, anchorPoint.y, anchorPoint.z);
